...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        self.screen_trading_stock = "6000" #종목별 할당할 주문용 스크린 번호
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "예수금상세현황요청":
            ...
            #[['','현재가', '거래량', '거래대금', '일자', '시가', '고가', '저가', ''], ['','현재가', '거래량', '거래대금', '일자', '시가', '고가', '저가', '']]
                ...
                if self.calcul_data == None or len(self.calcul_data) < 20:
                    pass_success = False
                else:
                    # 20일 이평선의 최근 가격 구함
                    total_price = 0
                    ...
                    # 오늘자 주가가 20일 이평선에 걸쳐있는지 확인
                    bottom_stock_price = False
                    check_price = None
                                #가장 최근일의 저가                                                            #가장 최근일의 고가
                    if int(self.calcul_data[0][7]) <= moving_average_price and moving_average_price <= int(self.calcul_data[0][6]):
                        print("오늘의 주가가 20 이평선에 걸쳐있는지 확인")
                        bottom_stock_price = True
                        check_price = int(self.calcul_data[0][6]) #가장 최근일의 고가

                    # 과거 일봉 데이터를 조회하면서 20일 이동평균선보다 주가가 계속 밑에 존재하는지 확인
                    prev_price = None
                    if bottom_stock_price == True:
                        ...
                        while True:
                            if len(self.calcul_data[idx:]) < 20: # 20일 치가 있는지 계속 확인
                                print("20일 치가 없음")
                                break
                            ...
                            if moving_average_price_prev <= int(self.calcul_data[idx][6]) and idx <= 2:
                                print("2일 동안 주가가 20일 이평선과 같거나 위에 있으면 조건 통과 못 함")
                                price_top_moving = False
                                break

                            elif int(self.calcul_data[idx][7]) > moving_average_price_prev and idx > 2: # 20일 이평선 위에 있는 구간 존재
                                print("20일치 이평선 위에 있는 구간 확인됨")
    ...
    def screen_number_setting(self):
        ...
        for code in screen_overwrite:
            ...
            if code in self.portfolio_stock_dict.keys():
                ...
                self.portfolio_stock_dict.update({code: {"스크린번호": str(self.screen_real_stock), "주문용스크린번호": str(self.screen_trading_stock)}})
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        if sRealType == "장시작시간":
            ...
            if sCode in self.account_stock_dict.keys() and sCode not in self.balance_dict.keys():
                ...
                if asd['매매가능수량'] > 0 and (trading_rate > 0.1 or trading_rate < -0.1):
                ...
                jd = self.balance_dict[sCode]
                trading_rate = (b - jd['매입단가']) / jd['매입단가'] * 100

                if jd['주문가능수량'] > 0 and (trading_rate > 0.1 or trading_rate < -0.1):
                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
            ...
            elif d > 0.5 and sCode not in self.balance_dict:
            ...
            not_trading_list = list(self.not_account_stock_dict)
            for order_num in not_trading_list:
                code = self.not_account_stock_dict[order_num]["종목코드"]
                trading_price = self.not_account_stock_dict[order_num]['주문가격']
                not_quantity = self.not_account_stock_dict[order_num]['미체결수량']
                order_gubun = self.not_account_stock_dict[order_num]['주문구분']

                if order_gubun == "매수" and not_quantity > 0 and e > trading_price:
                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
    ...
    def chejan_slot(self, sGubun, nItemCnt, sFidList):
        if int(sGubun) == 0: #주문체결
            ...
            trading_gubun = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['매도매수구분'])
            trading_gubun = self.realType.REALTYPE['매도수구분'][trading_gubun]
            ...
            self.balance_dict[sCode].update({"보유수량": stock_quan})
            self.balance_dict[sCode].update({"주문가능수량": like_quan})
            self.balance_dict[sCode].update({"매입단가": buy_price})
            self.balance_dict[sCode].update({"총매입가": total_buy_price})
            self.balance_dict[sCode].update({"매도매수구분": trading_gubun})
